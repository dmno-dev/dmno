...
The `@dmno/netlify-platform` module exposes DMNO data types and a pre-made config schema object which you can use in your own schema. You likely don't care about all of the env vars that Netlify injects, so you can use the `pickFromSchemaObject` utility to pick only the keys you care about. For example:
...
If you are building a totally static site, you probably do not need to use this build plugin!
:::
