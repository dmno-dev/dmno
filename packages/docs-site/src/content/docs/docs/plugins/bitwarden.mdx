...
After installation, you'll need to initialize the plugin in your dmno config and add a config item to hold your machine account access token. You can explicitly wire the plugin up to the service account token if using multiple tokens at once, or it will be injected by default. It's ok if you have not created the machine account or access token - we'll do that in the next section.
