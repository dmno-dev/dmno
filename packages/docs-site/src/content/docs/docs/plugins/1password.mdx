...
To get started with this method, create a new _Secure Note_ item within your vault. Give it a descriptive name (e.g., `Prod secrets`), then click `+ add more` and select `Text` to add a new multi-line text field. Change the default label of `text` to the [service name](/docs/guides/schema/#service-names) you want to store secrets for (e.g., `root`), and click `Save`.

Now we need to wire up our plugin instance to this new item using its _private link_. While viewing the item in the 1Password app, click the 3 dots in the top right and click `Copy Private Link`. As this link does not contain anything sensitive, we can use a static value as our plugin input.
