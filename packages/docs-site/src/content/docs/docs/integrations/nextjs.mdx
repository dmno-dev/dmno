...
Unlike some of our other integrations, the next integration requires that you run your next commands via [`dmno run`](/docs/reference/cli/run/). This means your package.json scripts that run `next` should be prefixed with `dmno run`. Our [`dmno init`](/docs/reference/cli/init/) cli will try to make these changes for you, but they should end up looking like this:
