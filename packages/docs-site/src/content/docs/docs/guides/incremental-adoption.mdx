...
**Feature by feature**: 
  ...
  - Next add types to each of the config items
  - Then add custom validation rules, coercion, and custom types
  - Finally, add [plugins](/docs/plugins/overview) to fetch the config values from a secure source

**In your code**: 
  - We will support your existing methods of using env vars (ex. `process.env` in Node.js, `import.meta.env` in Vite, etc) while you migrate to DMNO, by reinjecting your resolved config items (as strings). You won't get type completion, full intellisense, or validation. and you won't get an error if you try to access a variable that doesn't exist, unfortunately.
