...
To customize behavior, you can set `sesitive` to an object rather than `true`. Note that an empty object will still mark the item as being sensitive.
...
export default defineDmnoService({
  schema: {
    ...
    ONE_PASS_TOKEN: {
      // data types may already be marked as sensitive and have customized sttings
