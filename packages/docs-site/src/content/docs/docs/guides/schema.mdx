...
A typical DMNO monorepo project structure:

<FileTree>
- / (root of your project)
  - .dmno
    - **config.mts** (workspace/root config)
  - packages
    - api
      - .dmno 
        - **config.mts** (api service config)
    - frontend
      - .dmno
        - **config.mts** (frontend service config)
    - shared-lib
      - files... (no .dmno folder if no env vars are needed)
</FileTree>

:::note
There are many ways to organize package folders in a monorepo. We'll respect your monorepo's workspace settings (glob rules) for where to look (`package.json > workspaces`, `pnpm-workspace.yaml`, etc).
:::
...
This file must have `export default defineDmnoWorkspace({ //...`. Here is an example:
<br/>_(we'll get into more details below, so don't worry if some of it is confusing)_

```typescript title=".dmno/config.mts" "defineDmnoWorkspace"
import { defineDmnoWorkspace, NodeEnvType } from 'dmno';
...
- `pick` - pick specific config items from other services to use in this one (more on that below)
...
We recommend you set it to something short - for example `api` instead of `@my-cool-org/api` - because you may be typing it into CLI commands (ex: `pnpm exec dmno load -s api`) and it will be visible in several places in terminal output and our UI (COMING SOON).
...
Each item is defined by `extend`ing some base type (whether explicitly or not) and adding additional overrides on top of it.

Most of the time, you'll use existing data types, either from `DmnoBaseTypes` ([see reference](/docs/reference/base-types)) or from a published dmno plugin - some by dmno, some by others. These data types are factory functions, and accept settings that control reusable behavior (like validation rules). You should almost always be able to accomplish what you need with existing types - but of course you can author your own resuable types as well.
...
Again the syntax for `extend` is fairly rich, so best to illustrate via some examples:
```ts
export default defineDmnoService({
  schema: {
    ...
    // shorthand is provided for items w/o settings other than extends
    // (not recommended because attaching additional metadata/info is helpful)
...
export default defineDmnoService({
  ...
  schema: {
    SOME_API_KEY: {
      value: switchByNodeEnv({
        // some dev env key, so not a problem to expose
        _default: 'abc123',
