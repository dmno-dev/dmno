...
This file must have `export default defineDmnoWorkspace({ //...`. We'll get into more details below, so don't worry if some of it is confusing. Here is an example:



```typescript title=".dmno/config.mts" "defineDmnoWorkspace"
import { defineDmnoWorkspace, DmnoBaseTypes, NodeEnvType } from 'dmno';
...
We recommend you set it to something short - for example `api` instead of `@my-cool-org/api` - because you may be typing it into CLI commands (e.g., `pnpm exec dmno load -s api`) and it will be visible in several places in terminal output..
