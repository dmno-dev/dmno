...
```typescript title="[root]/config.mts"
...
```typescript title="services/child-service/config.mts"
...
<TabbedCode command="pnpm exec dmno plugin -p vault -- setup" />
...
<TabbedCode command="pnpm exec dmno plugin -p vault -- set-item" />

> **TODO** add tape gif

## Rotate the vault key

<TabbedCode command="pnpm exec dmno plugin -p vault -- rotate-key" />
