...
:::note[FYI]
Running `dmno init` will do this for you, creating the file either at the root of your project or in a `src` directory if you have one.

In many cases this will be enough and TypeScript will already pick up the new types.
:::


```ts title="src/dmno-env.ts"
...
:::caution
Bottom line is that if TypeScript doesn't somehow know about the `DMNO_CONFIG` globals, then while things may still work, you'll be missing out valuable help from your IDE.
:::
