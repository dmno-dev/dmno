...
First, install the `dmno` and `@dmno/dmno-next` packages.
...
> Simply use `DMNO_CONFIG` and `DMNO_PUBLIC_CONFIG` instead of `process.env` 🎉

### Security and secrets

While building your site, nextjs does 2 builds - one for the client and one for the server. During the client build, we only replace references to `DMNO_PUBLIC_CONFIG` (which never contains anything sensitive) so you can't accidentally leak any secrets into the JS of your site. Any references to `DMNO_CONFIG` in the client code will also throw a helpful error.
