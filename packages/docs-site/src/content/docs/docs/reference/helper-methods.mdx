...
> See the [root service config](/docs/guides/schema#root-service-config) in the schema guide for more information.
...
type ConfigItemDefinition = {
  ...
  exportEnvKey?: string,
  expose?: boolean,
  extends?: DmnobaseTypes | string | function,
    ...
    <PropTable properties={
      [
        ...
        },
        {
          name: 'exportEnvKey',
          type: 'string',
          description: 'The key to use when exporting the item to an env var.'
        },
        {
          name: 'expose',
          type: 'boolean',
          description: 'Whether the item should be exposed to other services. i.e. is it "pickable"'
        },
        {
          name: 'extends',
          type: 'DmnobaseTypes | string | function',
          description: 'A string (the name of the type), a reference to a type, or a function that returns a type'
        },
        {
          name: 'externalDocs',
        ...
        },
        {
          name: 'importEnvKey',
          type: 'string',
          description: 'The key to use when importing the item from the environment.'
        },
        {
          name: 'required',
          type: 'boolean',
...
> See the [service config](/docs/guides/schema#other-services) in the schema guide for more information.
...
This method is used to define different configurations for different values of a particular config item. Its arguments are a string (the key name) and an object with the following properties:
...
Example: 
```javascript
import { switchBy } from '@dmno/core';
