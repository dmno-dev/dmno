...
- ability to sync secrets with various backends (ex: [1password](/docs/plugins/1password)) or [encrypt](/docs/plugins/encrypted-vault) them within your repo - _no more insecurely passing around `.env` files via Slack_ 🎉
...
Look, we realize there are no shortage of tools out there to help manage your environment variables. And for most people, it's a set it up and forget it type of thing. But even if it's not top of mind at the moment, we guarantee it comes up _regularly_ - like each time you onboard a new team member, set up a new internal service, add a new external SaaS, or someone forgets to update some API key in the 12 different external systems.
