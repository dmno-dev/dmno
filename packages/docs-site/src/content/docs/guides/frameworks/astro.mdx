...
```diff lang="json" title="tsconfig.json" {"You only need to add the file types you are using:": 4} {"Add these references to dmno's auto-generated config schema:": 9}
{
  "extends": "astro/tsconfigs/strict",
+  "include": [

+    "src/**/*.astro",
+    "src/**/*.ts", "src/**/*.js", "src/**/*.d.ts",
+    "src/**/*.jsx", "src/**/*.tsx",
+    "src/**/*.mdx",
...
- In the browser/client, you only have access to `DMNO_PUBLIC_CONFIG` and if you try to access `DMNO_CONFIG`, we'll throw an error 🛑
...
In general accessing `DMNO_PUBLIC_CONFIG.FOO` is treated as static, and these vars get rewritten (by vite) at build time. This can be useful because it affects tree-shaking and can remove unreachable code.

Meanwhile `DMNO_CONFIG.BAR` is not rewritten at build time and will take the latest config into account - including overrides from system environment vars on the server at boot time. This can be useful for running a single build in different contexts...

We've got some tricks up our sleeve to help give you more control over this... coming very soon!
...
### Accessing DMNO_CONFIG within astro.config.mjs
...
However to get TS autocompletion in our .mjs file, we need to add a triple-slash reference to the dmno generated types.
...
export default defineConfig({
  ...
  integrations: [
    ...
    starlight({
      social: {
        // now we can use our config instead of hard-coding values 🥳
-        github: 'https://gihthub.com/dmno-dev/core',
...
Markdown files are processed by astro but treated as just pure content without evaluating any javsascript... So if you want to inject any DMNO_CONFIG values, you'll need to use MDX and follow some guidelines:

- install the [@astrojs/
mdx integration](https://docs.astro.build/en/guides/integrations-guide/mdx/) (`npx astro add mdx`)
- rename your `.md` file into a `.mdx` file

Then you need to use JSX within your markdown content:

For injecting text, that just looks like
```mdx
Some text: { DMNO_PUBLIC_CONFIG.SOME_VAR }
```

For links, you'll need to use the html/jsx version rather than a markdown style link:
```mdx {"🥳 This works!": 1} del={"🥵 These dont work": 4-7}
.
  <a href={DMNO_PUBLIC_CONFIG.DISCORD_JOIN_URL}>Discord</a>


  [Discord](DMNO_PUBLIC_CONFIG.DISCORD_JOIN_URL)
  [Discord]({DMNO_PUBLIC_CONFIG.DISCORD_JOIN_URL})
  [Discord]({ DMNO_PUBLIC_CONFIG.DISCORD_JOIN_URL })
