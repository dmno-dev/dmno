...
<TabbedCode packageName="@dmno/config-engine @dmno/dmno-cli @dmno/dmno-next" />
...
```typescript  
import {
  DmnoBaseTypes, defineWorkspaceConfig, configPath, switchByNodeEnv, dmnoFormula, asdf,
} from '@dmno/core';
...
```typescript
import {
  DmnoBaseTypes, defineConfigSchema, configPath, switchByNodeEnv, dmnoFormula,
} from '@dmno/core';
