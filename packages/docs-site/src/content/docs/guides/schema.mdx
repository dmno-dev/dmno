...
If you haven't already, follow our [Quickstart](/get-started/quickstart) guide to get started with DMNO. Once you're set up and ready to go, let's dive in to writing a full schema for your project.
...
```typescript
import { defineWorkspaceConfig } from '@dmno/dmno-core';
...
```typescript
import { defineConfigSchema } from '@dmno/dmno-core';
...
For an exhaustive list of all the options available for each schema item, check out the [defineConfigSchema](/reference/config-engine/helper-methods/#defineconfigschema) reference docs.
